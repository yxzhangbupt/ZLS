 ...
 */
var xhr=createXHR();
xhr.open("POST","./update.jsp",true);
xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');

var canvas = document.getElementById("canvas_QS"),
    context = canvas.getContext("2d"),
    RADIUS = 0.5,
    verticalButton=document.getElementById("btn_vertical"),
...
SCpainter = function (data_SC) {
    ...
    this.innerLeft=data_SC.innerLeft;   //当前水池底面积的位置和宽高
    this.innerTop=data_SC.innerTop;
    this.innerWidth=data_SC.innerWidth;
    this.innerHeight=data_SC.innerHeight;
    this.outerLeft=data_SC.outerLeft;   //水池充满时的面积和宽高
    this.outerTop=data_SC.outerTop;
    this.outerWidth=data_SC.outerWidth;
    this.outerHeight=data_SC.outerHeight;
    this.percentage=0.1;
...
SCpainter.prototype = {
    paint: function (sprite, context) {
        ...
        if(this.percentage==0.0){
            ...
            var h = parseFloat(this.percentage*5).toFixed(1);
            document.getElementById(this.readDiv).innerHTML = "高度：" + h + "M";
            context.fillRect(this.innerLeft-(this.innerLeft-this.outerLeft)*this.percentage,this.innerTop-(this.innerTop-this.outerTop)*this.percentage,
                this.innerWidth+(this.outerWidth-this.innerWidth)*this.percentage,this.innerHeight+(this.outerHeight-this.innerHeight)*this.percentage);
        }
...
//            //this.height = 0;
//        }
//        if (this.height > this.SCheight * 0.9) {
//            context.fillStyle = "#9abbe8";
//            if (this.height % 2) {
//                context.fillRect(sprite.left, sprite.top + this.SCheight - this.height, this.SCwidth, this.height);
//            }
...
verticalButton.onclick=function(e){
...
//            console.log(parseFloat(returnText));
//            SC01.percentage=0.5;
////            SC01.percentage=parseFloat(returnText);
//        }
//    }
//    xhr.open("POST","update.jsp",false);
//    xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
//    data_send="PoolId=MTG_QS_SC01";
//    xhr.send(data_send);
    $.get("update.jsp",{PoolId:"MTG_QS_SC01"},function(data){
//            SC01.percentage=parseFloat(data);
            SC01.percentage=0.5;
...
var paused = true,
    cells = [
        {x: 250, y: 0, velocityX: 2, velocityY: 0},
        {x: 250, y: 269, velocityX: 2, velocityY: 0},
        {x: 345, y: 269, velocityX: 0, velocityY: 3},
        {x: 345, y: 536, velocityX: 0, velocityY: 3},
        {x: 410, y: 536, velocityX: 0, velocityY: 3}
    ],
    cells1 = [
        {x: 345, y: 0, velocityX: 1, velocityY: 0},
        {x: 345, y: 374, velocityX: 0, velocityY: 3},
        {x: 410, y: 374, velocityX: 0, velocityY: 3}
    ],
    cells2 = [
        {x: 847, y: 411, velocityX: 1, velocityY: 0},
        {x: 958, y: 411, velocityX: 1, velocityY: 1}
    ],
    cells3 = [
        {x: 847, y: 521, velocityX: 1, velocityY: 0},
        {x: 958, y: 521, velocityX: 1, velocityY: 1}
    ],
    cells4 = [
